...
```
blessing-ai/
├── docs/                  ← le code de l'application (c'est ce que Capacitor empaquette)
│   ├── index.html
│   ├── css/style.css
...
Ouvre simplement `docs/index.html` dans ton navigateur, ou lance un petit serveur :
...
Tout le contenu (versets, prières, méditations, thèmes) est centralisé dans `docs/js/data.js`. Pour ajouter un verset, une prière ou un contexte, il suffit d'ajouter une entrée dans le tableau correspondant — aucune autre modification n'est nécessaire.
